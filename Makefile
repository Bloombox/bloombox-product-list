...
#
# - Bloombox - bloombox-product-list
#
...
all: build
	@echo "bloombox-product-list is ready."
...
ifeq ($(BUILDBOT),yes)
test:
	wct --job-name "bloombox-product-list" --expanded --build-number "$(BUILD_NUMBER)" --sauce-tunnel-id "$(SAUCE_TUNNEL)" --plugin wct-jenkins
else
test:
	@wct --job-name "bloombox-product-list"
endif
